...
function initJMuxer() {
  ...
  // Smooth Dynamic Catch-Up (Prevents seeking stutter and continuous micro-freezes)
  playerEl.addEventListener('timeupdate', () => {
    if (playerEl.buffered && playerEl.buffered.length > 0) {
      ...
      if (lag > 2.0) {
        // Severe network lag (>2s): Hard seek to live edge
        playerEl.currentTime = bufferEnd - 0.1;
        playerEl.playbackRate = 1.0;
      } else if (lag > 0.4) {
        // Minor drift (0.4s - 2.0s): Smoothly speed up by 15% without decoder stutter
        playerEl.playbackRate = 1.15;
      } else if (lag < 0.15) {
        // Caught up to real-time: Resume normal 1.0x speed
        playerEl.playbackRate = 1.0;
      }
    }
  });
